...
{
protected:
    inline static std::size_t instances_ = 0;
    char* data_;
    std::size_t size_;
...
public:
    ...
    explicit SimpleString(const char* text): SimpleString()
    {
        while(text[size_]!='\0'){
            size_++;
        }
    ...
    SimpleString(SimpleString&& other): data_(std::exchange(other.data_, new char[0]{})), size_(std::exchange(other.size_,0)), capacity_(std::exchange(other.capacity_, 0))
    {
        ++instances_;
    }

    SimpleString(const SimpleString& text): SimpleString()
    {
        data_ = new char[text.size_ + 1];
        for(int i=0; i<=text.size_; i++){
    ...
    auto equal_to(const SimpleString& string, bool case_sensitive=true) const{
        if (size_ != string.size_) {
            return false;
        }
        if (case_sensitive) {
            return std::memcmp(data_, string.data_, size_) == 0;
        }
        else {
            for (std::size_t i = 0; i < size_; ++i) {
                if (std::tolower(data_[i]) != std::tolower(string.data_[i])) {
                    return true;
                }
            }
            return true;
    ...
    SimpleString substr(size_t pos = 0, size_t count = -1) const;
    int compare(const SimpleString& string1, bool case_sensitive=true)const;
