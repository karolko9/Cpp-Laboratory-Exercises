...
public:
    ...
    VectorWrapper(const value_type elements[], std::size_t N) : impl_(elements, elements+N) {}

    void push_back(const value_type& a) override {
        impl_.push_back(a);
    }
    void push_front(const value_type& a) override {
        //impl_.push_front(a);
    }

    void insert(const value_type& a, std::size_t s) override {
        impl_.insert(impl_.begin()+s, a);
    }

    std::size_t size() const override { return impl_.size(); }

    value_type& at(std::size_t s) override
    {
        return impl_.at(s);
    }
    ...
    void erase(std::size_t s) override {
        impl_.erase(impl_.begin()+s);
    }

    value_type count() const override { return std::accumulate(impl_.begin(), impl_.end(),value_type (0)); }

    std::size_t find(const value_type& s) const override{
        auto i = std::find(impl_.begin(), impl_.end(), s);
        return i != impl_.end() ? i - impl_.begin() : static_cast<size_t>(-1);
    }

    value_type pop_front() override {
        value_type temp = impl_.front();
        for(int i=0; i<impl_.size()-1; i++){
            impl_[i]=impl_[i+1];
        }
        impl_.pop_back();
        return temp;
...
class VectorPreallocatedWrapper: IContainerWrapper
{
